...
export GOCACHE := $(YAMLARK_CACHE)/GOCACHE
.PHONY: GOCACHE
GOCACHE:
	@echo $(GOCACHE)
export GOMODCACHE := $(YAMLARK_CACHE)/GOMODCACHE
...
YAMLARK_PATTERN ?= '**/*.{go},Makefile'
YAMLARK_EXTRA_CMD ?= true
...
build: go go-generate
	$(GO) build -o yamlark.$(GOOS).$(GOARCH) $(GO_BUILD_FLAGS) .
...
.PHONY: yamlark-help
yamlark-help:
	@echo 'yamlark: rerun build automatically on file changes then runs YAMLARK_EXTRA_CMD'
help: yamlark-help

.PHONY: yamlark
yamlark: go
	$(YAMLARK) \
		--debounce $(YAMLARK_DEBOUNCE) \
		--ignore-pattern $(YAMLARK_IGNORE_PATTERN) \
		--log-level $(YAMLARK_LOG_LEVEL) \
		--pattern $(YAMLARK_PATTERN) \
		-- \
		sh -c "$(MAKE) $(MFLAGS) ci && $(YAMLARK_EXTRA_CMD)"
